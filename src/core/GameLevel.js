...
class GameLevel extends NOVA.World {
  constructor(app, clearColor, data, settings) {
    const halfWidth = app.getWorldWidth() / 2;
    const halfHeight = app.getWorldHeight() / 2;
  ...
  setCharactor(charactor) {
    ...
    const sp = this.data.path[this.data.startPoint];
  ...
  loadResource() {
    ...
    for (const item of this.data.textures) {
  ...
  init() {
    ...
    for (const key in this.data.path) {
  ...
  initLevelBoard(levelBoard) {
    ...
    const w = this.app.getWorldWidth();
    const h = this.app.getWorldHeight();
    ...
    for (const item of levelBoard.info) {
      if (item.type == 'text') {
        ctx.font = `${item.weight} ${item.size * h}px ${item.family}`;
        ctx.fillStyle = item.color;
        const { width } = ctx.measureText(item.text);
        ctx.fillText(item.text, w / 2, h * item.y);
      } else if (item.type == 'pic') {
        const imgWidth = this.loaderFactory.Resource.textures[item.src].image.naturalWidth;
        const imgHeight = this.loaderFactory.Resource.textures[item.src].image.naturalHeight;
        const newHeight = item.height * h;
  ...
  createPathToScene(pathInfo) {
    ...
    if (pathInfo.materialId) {
      ...
      for (const i in this.data.materials) {
        m = this.data.materials[i];
        break;
      }
    }
    const obj = new THREE.Mesh(this.geometryResource.groundGeometry, m);
  ...
  clickCommonEvent(obj) {
    ...
    if (this.charactor.isWalking == false) {
      ...
      const path = this.graphPath.findPath(this.charactor.walkingPath[0], obj.pathId);
      if (path === false) {
        this.charactor.walkingPath = [this.charactor.walkingPath[0]];
        return;
      }
  ...
  moveCharacter(arr) {
    ...
    const nextP = this.data.path[arr[0]];
    ...
    if (nextP.parentId && !nextP.local) {
      ...
      if (!this.charactor.isLocal) {
        this.charactor.parentId = nextP.parentId;
        const parent = this.childrenWithId[nextP.parentId];
    ...
    } else if (this.charactor.isLocal) {
      this.charactor.isLocal = false;
    ...
    let time = this.settings.moveSpeed;

    const prevP = this.data.path[this.charactor.currentPath];
    vec.y -= 0.5 * size;

    if (prevP.changeSpeed && prevP.changeSpeed[nextP.id]) {
      const str = prevP.changeSpeed[nextP.id];
    ...
    let canMove = false;
    for (const item of prevP.neighbors) {
      if (item == nextP.id) {
        canMove = true;
      }
    }
  ...
  calculateFaceAngle(charactor, nextPoint) {
    const vec = charactor.position.clone();
    const delta = nextPoint.y - vec.y;
    vec.x += delta;
    vec.z += delta;
    const EPSILON = 0.01;
    const dx = nextPoint.x - vec.x;
    const dz = nextPoint.z - vec.z;
